test_that("Test that insert_protocolsection works", {
  ...
  write(
    x = test_params,
    file = "source/sfp/1_water/sfp-101-nl_water-1/07_werkwijze.Rmd",
    append = TRUE)
  # add the protocolspecific parameter to index yaml
  index <- readLines(
    "source/sfp/1_water/sfp-101-nl_water-1/index.Rmd")
  index <- c(index[1:14],
             "  protocolspecific: defaultvalue",
             index[15:length(index)])
  writeLines(index,
             con = "source/sfp/1_water/sfp-101-nl_water-1/index.Rmd")
